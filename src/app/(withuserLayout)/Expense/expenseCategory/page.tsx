  ...
  const [categories, setCategories] = useState<Category[]>([
    {
      ...
      description: "Expenses for office-related supplies",
      amount: 20, // Updated to a number
      status: true,
    },
    {
      id: 2,
  ...
  return (
    <div className="w-full pl-[1.5%] pr-[3.5%]">
      ...
      <div className="flex justify-end mb-4">
        <button
          onClick={toggleModal}
          className="px-5 py-2 relative rounded-lg group overflow-hidden font-medium bg-indigo-100 text-indigo-600 flex items-center justify-center">
          <span
            className="absolute top-0 left-0 flex w-full h-0 mb-0 transition-all duration-200 ease-out transform translate-y-0 bg-indigo-600 group-hover:h-full opacity-90"></span>
          <span className="relative group-hover:text-white text-base font-semibold">Add Category</span>
        </button>
      </div>
      ...
      {isOpen && (
        <div className="fixed top-0 left-0 z-[100] w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white rounded-2xl shadow-lg w-full sm:max-w-lg p-6">
            {/* Modal Header */}
            <div className="flex justify-between items-center pb-4 border-b border-gray-200">
              <h4 className="text-sm text-gray-900 font-medium">Add Expense Category</h4>
              ...
              <div className="py-4">
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    className="block w-full px-4 py-2 text-base border rounded-lg shadow-sm focus:outline-none focus:ring focus:ring-indigo-200"
                  />
                  {errors.name && (
      ...
      <div className="w-full bg-white shadow-md rounded-lg">
       ...
       <table className="w-full border text-sm text-left text-gray-700">
            <thead className="border-b text-gray-600 text-[13px] uppercase bg-gray-10">
              <tr>
                <th className="px-6 py-3">Category Name</th>
                <th className="px-6 py-3">Category Description</th>
            ...
            <tbody>
              {categories.map((category) => (
                <tr key={category.id} className="bg-white border-b hover:bg-gray-50">
                  <td className="px-6 text-gray-500 py-4">{category.name}</td>
                  <td className="px-6 py-4 text-gray-500">{category.description}</td>
                  <td className="px-6 py-4 text-indigo-600 font-medium">${category.amount}</td>
                  <td className="px-6 py-4">
                    <div
                      className={`py-1.5 px-2.5 rounded-full flex justify-center w-20 items-center gap-1 ${category.status ? "bg-emerald-50" : "bg-red-50"
                        }`}
                    >
                      <span
                        onClick={() => toggleStatus(category.id)}
                        className={`font-medium text-xs cursor-pointer ${category.status ? "text-emerald-600" : "text-red-600"
                          }`}
                      >
